This respository contains the python notebooks that are used for all the analysis of aperiodic analysis in sleep paper 

The notebooks use toolboxes that are specificed in the dependencies, but also some custom made funtions for statistical testings anf plotting that are founf in the helper folder in the utils.py and plots.py
...
Simulate an electrophysiological signal and fits the differnt models to it, then compares the parameters of the different models
...
## 05_iEEG_PSDs
Compute PSDs of the different sleep stages of the iEEG data (Average PSD, Region PSD, Stage PSD)
...
## 07_iEEG_decoding:
Uses an LDA classifier to differentiate between sleep stages using Knee Frequency, exponent of the knee model and exponent of the fixed model, compares the results, then identifies the parameter that yields the best performance.

## 08_iEEG_model_comparison:
Compares between the performance of the different models fit to the iEEG using the byesian infomration criterior (BIC)

# 3. EEG analysis

## 09_EEG_fit_specparam:
Fit aperiodic models (Knee and Fixed) to the different sleep stages of the EEG data.

## 10_EEG_PSDs
Compute PSDs of the different sleep stages of the EEG data (Average PSD, Region PSD, Stage PSD)
...
Compares between the performance of the different models fit to the EEG using the byesian infomration criterior (BIC)

# 4. Time-resolved analysis in full night continous EEG data

## 15_EEG_WholeNight_SingleSubj:
Plots the time-frequency representation, the hypnogram, as well as the exponent over the whole night in one subject of the EEG experiment (for estimation script see: Python_scripts/Exp_TF_SingleSubj)
